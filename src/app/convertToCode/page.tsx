...
export default function ConvertToCode() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (!trimmed) {
        setOutput("// لا يوجد نص لتحويله — الرجاء إدخال نص في الصندوق الأيسر ثم اضغط Generate.");
        return;
      }
      ...
      if (lower.includes("component") || lower.includes("react")) {
        result = `// React component generated (example)\n\nimport React from "react";\n\nexport default function GeneratedComponent() {\n  return (\n    <div>\n      ${trimmed.replace(/\n/g, "\n      ")}\n    </div>\n  );\n}\n`;
      } else if (lower.includes("function") || lower.includes("fn")) {
        result = `// Function skeleton\n\nfunction generated() {\n  ${trimmed.replace(/\n/g, "\n  ")}\n}\n`;
      } else {
        result = `// Converted (example)\n\n/* Original: */\n/* ${trimmed.replace(/\n/g, "\n * ")} */\n\n// Example output (JavaScript)\nconsole.log(${JSON.stringify(trimmed)});\n`;
      }
  ...
  return (
    <div className=" flex flex-col items-center justify-start py-12 px-4 sm:px-8 ">
      <div className="w-full max-w-6xl">
        <h1 className="text-4xl font-bold mb-4 font-Alexandria text-right">صفحة تحويل من كتابة إلى كود</h1>
        <p className="text-lg text-white mb-8 font-Cairo  text-right">
          هنا يمكنك كتابة وصف أو نص في الصندوق الأيسر ثم الضغط على زر <strong>Generate</strong> لتحصل على كود في الصندوق الأيمن.
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col">
            <label className="mb-2 text-sm font-medium text-slate-700">Input — اكتب الوصف هنا</label>
            <textarea
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder="مثال: component that shows a user card with avatar and name..."
          ...
          <div className="flex flex-col">
            <label className="mb-2 text-sm font-medium text-slate-700">Output — الكود الناتج</label>
            ...
            <div className="flex items-center justify-between mt-3">
              <div className="flex gap-2">
                <button
                  onClick={copyOutput}
                  disabled={!output}
                  className={`px-3 py-1.5 text-sm rounded-md border ${output ? "bg-amber-100 hover:bg-amber-200" : "bg-slate-50 text-slate-400 cursor-not-allowed"}`}
                >
                  Copy
                </button>
                <button
              ...
              <div className="text-sm text-slate-500">{output ? `${output.split("\n").length} lines` : ""}</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 flex items-center justify-end gap-3">
          ...
          <button
            onClick={generate}
            disabled={loading}
            className={`px-6 py-3 rounded-md text-white font-medium ${loading ? "bg-amber-300 cursor-wait" : "bg-amber-500 hover:bg-amber-600"}`}
          >
