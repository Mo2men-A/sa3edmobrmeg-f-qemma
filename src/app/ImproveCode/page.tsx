...
export default function ImprovePage() {
  ...
  const handleImprove = async () => {
    ...
    try {
      const response = await fetch("/api/ImproveCode", {
        method: "POST",
      ...
      if (response.ok && data.patch) {
        setImprovedCode(data.patch.trim());
      } else {
        setError(data.error || "حدث خطأ غير معروف");
      }
    } catch (err) {
      setError("فشل الاتصال بالـ API. حاول مرة أخرى.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="container mx-auto p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          <h2 className="text-xl font-semibold mb-2 font-Alexandria text-right">
            ادخل الكود
          </h2>
          <textarea
            rows={15}
        ...
        <div>
          ...
          <div className="relative">
            ...
            <pre className="w-full h-120 p-4 bg-gray-900 text-yellow-300 rounded-md border border-gray-700 overflow-auto font-mono">
              {improvedCode}
            </pre>
          </div>
        </div>
      ...
      <button
        ...
        disabled={isLoading}
        className="mt-20 bg-[#556244] text-black py-5 px-6 rounded-md font-semibold disabled:bg-gray-500 disabled:cursor-not-allowed transition-colors justify-center flex mx-auto font-Cairo"
      >
        {isLoading ? "جارٍ التحليل..." : "ابدأ التحسين"}
      </button>
    </div>
  );
}
